...
OpenWolf has four independently compiled parts that together form a working CLI:
...
2. **Hooks** (`tsc -p tsconfig.hooks.json`) — compiles `src/hooks/*.ts` into standalone Node scripts that Claude Code executes directly. Hooks run in isolation and cannot import from `src/utils/` at runtime; `src/hooks/shared.ts` is a thin barrel that re-exports utilities from seven internal `wolf-*` modules.
...
4. **Templates** (`pnpm build:templates` via `cp -r src/templates dist/templates`) — canonical `.wolf/` seed files copied verbatim to `dist/templates/` during build. These are the files that `openwolf init` copies into a project's `.wolf/` directory.
