...
| `execution_layer_note` | `"Optional: set to your execution layer name so OpenWolf can point resume at its plan/status on session start. When null the generic 3-step resume order applies (execution layer plan/status, then cerebrum.md, then memory.md)."` | Human-readable hint about what the key does. This key is informational only — OpenWolf does not read it at runtime. |
...
# `.wolf/hooks/` pattern — observed in acme_translators where a root rule masked
# `.wolf/hooks/` despite the per-file template). This file is the single source
# of truth for `.wolf/` tracking.
...
# Derived / regenerated locally — anatomy.md is rebuilt by `openwolf scan`,
# the daemon, or session-start self-heal. Committing it churned every session
# and leaked machine-local paths into shared git.
anatomy.md

# Derived from source (TypeScript to JavaScript); rebuilt by `openwolf init`,
...
# (buglog.ndjson is the authored append-only log and IS committed — see above.)
buglog.json
...
# NOT regenerated by scan — only updated via `openwolf learnings merge/accept`.
...
The template implements a "mixed commit strategy": authored shared files (cerebrum.md, OPENWOLF.md, config.json, identity.md, buglog.ndjson, cron-manifest.json, reframe-frameworks.md) are committed to git for team visibility, while per-developer state (memory, token ledger, design captures, sessions) and derived artifacts (anatomy.md, hooks/) remain ignored. The template uses a whitelist approach: files not listed in the ignore rules are committed by default.
