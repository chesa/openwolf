...
- [x] **Phase 12: Framework-Blind Curation Machinery** - Continuous capture, Git-boundary promotion gate, and cerebrum freshness integrity (R7a, R7b, R9) (completed 2026-06-26)
...
- [x] 11-02-PLAN.md — Delete `checkStatusFreshness()` from stop.ts; make wolf-ignore.ts JSDoc C1-clean; rebuild + copy the hook bundle (C1/C2 gates)
...
- [x] 12-01-PLAN.md — Create dep-free `src/hooks/wolf-pantry.ts`: relocate `collectAllEntries`/`parseProposals`/`ProposalEntry` with presence-based stub detection (D12-05b) + add the R9 `normalizeCerebrumBody`/`hashCerebrumBody` engine (TDD)
